...
int main(int argc, char* argv[])
{
	...
	do
	{
		...
		if (t > (time + 1000)) {
			if (MAZE[Y_Position +1][X_Position] == EMPTY_SPACE) // if empty space below
			{
				Y_Position++;
				draw_character(X_Position, Y_Position, AVATAR); // draws avatar on new line
				draw_character(X_Position, Y_Position-1,EMPTY_SPACE); // draws empty space on old avatar location

			}
		if (MAZE[Y_Position][X_Position+1] == EMPTY_SPACE)
			{
			if (gx < -0.2) { // tilt function
				X_Position += 1;
				draw_character(X_Position, Y_Position, AVATAR);
			...
			// Bonus Questions
			if ((MAZE[Y_Position +1][X_Position] == WALL )&& (MAZE[Y_Position][X_Position - 1] == WALL) && (MAZE[Y_Position][X_Position + 1] == WALL)){
				break_out = 3;
				break;
				printf("YOU LOSE"); //Did we win?
			}
			for (int side = 0; side < NUMCOLS; side++) {
				if ((MAZE[Y_Position +1][X_Position] == WALL )&&(MAZE[Y_Position][X_Position - 1] == WALL) && (MAZE[Y_Position][X_Position + side] == WALL)){
					break_out = 3;
					break;
					printf("YOU LOSE" ); //Did we win?
				}
	...
	count += 1;
	if (Y_Position == NUMROWS - 1) { // breaks out of the loop when Avatar is at the 72nd row.
		break;
	}
...
float calc_roll(float x_mag){ // Didn't need to use
	if (x_mag > -1 && x_mag < 0) {
		x_mag = -1;
	}
	else if (x_mag > 1){
