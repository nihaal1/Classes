...
//        }
        public synchronized void extend(String var, Value val) {
            if (map.containsKey(var)) {  // Check if the variable already exists
                System.out.println("Re-declaration of variable \"" + var + "\" detected. Discarding redefinition.");
            } else {
                map.put(var, val);  // Only add the new variable if it is not already present
            }
}
    }
