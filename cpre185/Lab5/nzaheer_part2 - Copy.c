...
// FIXME: the curly braces for while (include fflush )
    ...
    while (close_to(0.2x, 1.0, mag(gx, gy, gz))) {
        scanf("%d, %lf, %lf, %lf", &t,&gx, &gy, &gz);
        count += 1;
        ...
        while (!close_to(0.75, 3.0, mag(gx, gy, gz))) { // now calcuate time difference and use that in equation to find distance
          ...
          //    tPrevious = (t - tPrevious)/1000.0;
              ...
              vCurrent = vCurrent + (9.8 * ((1 - mag(ax,ay,az))* (t1 - tPrevious)/1000.0) );
...
time_seconds = (t2 - t1)/1000.0; // now with this we use the equation x1 = x0 + vt + (1/2)gt^2 where g= 9.8
