...
int first_come_first_served(process proc[], int t)
{
  ...
  for (i = 0; i < NUM_PROCESSES; i++)
  {
    // t is current sim time, check if it is lesser (meaning arrived)
    if (proc[i].arrivaltime <= t && proc[i].finished == 0)
    {
      if (proc[i].arrivaltime < min_time)
      {
        min_time = proc[i].arrivaltime;
        min_index = i;
      }
    }
  }
...
int shortest_remaining_time(process proc[], int t)
{
  int i = 0;
  int rem_index = -1;
  int rem_time = __INT_MAX__;
  for (i = 0; i < NUM_PROCESSES; i++)
  {
    // check if arrived
    if (proc[i].arrivaltime <= t && proc[i].finished == 0)
    {
      if (proc[i].remainingtime < rem_time)
      {
        rem_time = proc[i].remainingtime;
        rem_index = i;
      }
    }
  }
...
int round_robin(process proc[], int t)
{
  ...
  {
    for (i = prev_index + 1; i < NUM_PROCESSES; i++)
    {
      if (proc[i].arrivaltime <= t && proc[i].finished == 0)
      {
        prev_index = i;
...
int round_robin_priority(process proc[], int t)
{
  ...
  for (i = 0; i < NUM_PROCESSES; i++)
  {
    if (proc[i].arrivaltime <= t && proc[i].finished == 0)
    {
      ...
      if (proc[i].priority > proc[prev_index].priority)
      {
        prev_index = i;
      }
      else if (proc[i].priority == proc[prev_index].priority)
      {
        if (rem_time[i] < rem_time[prev_index])
        {
          prev_index = i;
        }
      }
