...
int PRAlgo_FIFO(const PageFrame *PageFrames, int num_frames, const int *PageAccesses, int num_accesses, int current_access)
{
	...
	int i;
	int FIFO_time_of_arrival;
	FIFO_time_of_arrival = PageFrames[0].time_of_arrival;
	for (i = 1; i < num_frames; i++)
	{
		if (PageFrames[i].time_of_arrival < FIFO_time_of_arrival)
		{
			FIFO_time_of_arrival = PageFrames[i].time_of_arrival;
			frame_to_evict = i;
		}
	}
...
int PRAlgo_LRU(const PageFrame *PageFrames, int num_frames, const int *PageAccesses, int num_accesses, int current_access)
{
	int frame_to_evict = 0;
	int i;
	int LRU_time_of_access;
	LRU_time_of_access = PageFrames[0].time_of_access;

	for (i = 1; i < num_frames; i++)
	{
		if (PageFrames[i].time_of_access < LRU_time_of_access)
		{
			LRU_time_of_access = PageFrames[i].time_of_access;
			frame_to_evict = i;
		}
	}
	// frame_to_evict = i;
...
int PRAlgo_OPT(const PageFrame *PageFrames, int num_frames, const int *PageAccesses, int num_accesses, int current_access)
{
	...
	int i, j;
	int OPT_id;
	int greatest_frame_j = -1;

	for (i = 0; i < num_frames; i++)
	{
		OPT_id = PageFrames[i].page_id;
		// greatest_frame_j = num_accesses;
		// PageFrames[i].time_of_access
		for (j = current_access; j < num_accesses; j++)
		{

			if (OPT_id == PageAccesses[j])
			{
				if (j > greatest_frame_j)
				{
					greatest_frame_j = j;
					frame_to_evict = i;
				}
			...
			else if (j == num_accesses - 1)
			{
				greatest_frame_j = __INT_MAX__;
				frame_to_evict = i;
				break;
			}
		}
		if (greatest_frame_j == __INT_MAX__)
		{
			break;
		}
	}
