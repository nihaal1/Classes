...
// Converts two characters to an unsigned short with two, one
unsigned short shortSwap(unsigned short one, unsigned short two)
{
    unsigned int res = two;
    res << 16;
    res += one;
    return res;
}
...
void parseDirectory(int iDirOff, int iEntries, unsigned char buffer[])
{
    ...
    for (i = 0; i < (iEntries); i = i + 1)
    {
        ...
        // Display filename
        toDOSName(string, buffer, (i * 32) + 0 /*TODO: replace 0 with correct value */);
        printf("%-12s\t", string);
        ...
        parseTime(string, endianSwap(buffer[(i * 32) + 22],buffer[(i * 32) + 23]) /*TODO: replace 0 with correct value */);
        printf("%s\t", string);
        ...
        // Display Size
        swap1 = endianSwap(buffer[(i * 32) + 28],buffer[(i * 32) + 29]);
        swap2 = endianSwap(buffer[(i * 32) +30],buffer[(i * 32) + 31]);
        


        printf("%d\n", shortSwap(swap1,swap2) /* TODO: replace 0 with actual size */);
    }
